...
echo "seq_len,hidden_size,layers,heads,recall@10,ndcg@10" > $LOG_FILE

for seq_len in "${SEQ_LENS[@]}"
do
  MODEL_FILE="bert4rec_seq${seq_len}.pth"

  echo "▶️ Training seq_len=$seq_len (hidden_size=$HIDDEN_SIZE, layers=$LAYERS, heads=$HEADS)"
  ...
  echo "$seq_len,$HIDDEN_SIZE,$LAYERS,$HEADS,$RECALL,$NDCG" >> $LOG_FILE
